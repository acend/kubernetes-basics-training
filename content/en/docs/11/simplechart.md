...
{{% onlyWhen mobi %}}
Because you cannot pull the `nginx` container image on your cluster, you have to use the `REGISTRY/puzzle/k8s/kurs/nginx` container image. Change your `mychart/values.yaml` to match the following:

```yaml
[...]
image:
  repository: REGISTRY/puzzle/k8s/kurs/nginx
  tag: stable
